...
единый поток, и `run_id` диспатченного event'а совпадает с
LangChain-run'ом, в котором он произошёл. Авто-вкладывание спанов
(например, Langfuse) в tool-span — задача интеграции конкретного
бэкенда: `BaseCallbackHandler.on_custom_event` даёт вам `run_id`, а
дальше handler решает, что с ним делать. Для авто-нестинга в
Langfuse подключите `langfuse.langchain.CallbackHandler` рядом с
`LangfuseHook` в одном списке `callbacks` — он выставляет OTel
контекст, на который опирается `client.start_observation`.
...
| `new_edges` | `list[FactEdge]` | По одному на каждое occurrence факта в модели. Рёбра append-only с уникальными id, так что это «все рёбра, произведённые этим ингестом» — `linked_edges` нет, потому что не нужен. |
...
```bash
uv add 'agent-pinboard[langfuse]'        # или: pip install agent-pinboard[langfuse]
```
...
Авто-вкладывание спанов под LangChain tool-span — **не
автоматическое**: Langfuse SDK определяет parent через OpenTelemetry
context, который выставляет `langfuse.langchain.CallbackHandler`.
Чтобы получить связный трейс, подключите оба handler-а в один
`callbacks`:
...
Без `LangfuseLangChain` спаны AgentPinBoard уйдут в Langfuse, но
отдельными top-level трейсами.
...
```bash
uv add 'agent-pinboard[ws]'        # или: pip install agent-pinboard[ws]
```
...
Cross-process distributed lock для **графовых** namespace'ов
(nodes/edges) не нужен: storage-модель mergeable by construction.

**Caveat для namespace `tool_calls`.** `ToolCallRecord` — append-only
с timestamp-derived ключами; в multi-process сценарии два воркера
могут записать record на «один и тот же» вызов без дедупликации.
`what_have_i_done` и warning про `tool_log_soft_limit` посчитают оба,
а дедупликация `on_duplicate=OnDuplicate.SKIP` будет best-effort
между процессами (каждый воркер видит только свой фрагмент лога до
следующего `load_tool_calls`). Если нужна строгая дедупликация
tool-лога между воркерами — оставляйте `on_duplicate=ALWAYS` и
полагайтесь на agent-loop не повторять идентичные вызовы.
