...
# ✗ кидает PinBoardConfigError при декорировании
@tool
@fact(model=X)
def f(...): ...
```

`@tool` создаёт `BaseTool`; `@fact` его оборачивает. Обратный порядок
— и `@fact` получает функцию вместо Tool. Спека реджектит сразу,
...
Реджектится в `register_model` через `PinBoardConfigError`. `BaseModel`
...
Каждый `@fact`-вызов нуждается в `runtime.store`, который выставляется
только при `.compile(store=...)`. Без него первый же tool-вызов
кидает `PinBoardConfigError("graph must be compiled with .compile(store=...)")`.
...
Если `runtime.config.configurable.thread_id` не задан — PinBoard
