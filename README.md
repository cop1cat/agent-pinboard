# AgentPinBoard — Техническое задание
...
- **AgentPinBoard — невидимый side-effect.** Декоратор `@pin` не изменяет return тула. Граф строится в фоне. Пользователь может опционально настроить поведение через хуки и `response_transform`.
...
│       (промпты, StateGraph, выбор модели — вне AgentPinBoard)    │
└──────────────────────┬──────────────────────────────────────┘
                       │ использует
┌──────────────────────▼──────────────────────────────────────┐
│                  AgentPinBoard (библиотека)                      │
│                                                             │
│   @pin декоратор → извлечение фактов из return тула        │
...
- `description` непустой — иначе `AgentPinBoardConfigError`.
- `type` — инстанс `Entity`, не строка — иначе `AgentPinBoardConfigError` с подсказкой «pass an Entity instance, not a string».
- тип поля — примитив или `list[primitive]`; `BaseModel`-поля с `node()` — запрещены, проверяется при `register_model` (`AgentPinBoardConfigError`).

Возвращает обычный `FieldInfo` с AgentPinBoard-метаданными в `json_schema_extra`. Pydantic работает с моделью как с обычной.
...
На каждый вызов тула, обёрнутого `@pin`, создаётся ровно одна `EventNode`. При `many=True` — по одной на каждый элемент возвращённого списка.
...
`@pin` оборачивает `@tool`. После вызова тула (sync или async):
...
**Порядок декораторов.** `@pin` всегда выше `@tool`. Python применяет декораторы снизу вверх: `@tool` создаёт `BaseTool` из функции, затем `@pin` оборачивает этот `BaseTool` — подменяет `invoke`/`ainvoke` на свою логику, сохраняя интерфейс LangChain-тула. Обратный порядок невалиден: `@tool` не ожидает видеть уже обёрнутую функцию.
...
```python
@pin(model=VTReport, mask_args=["api_key", "token"])
@tool
...
**Async.** Если тул async, а `response_transform` sync — вызывается напрямую в том же цикле. Если sync тул и async `response_transform` — AgentPinBoard бросит `AgentPinBoardConfigError` при регистрации (нельзя await-нуть из sync-контекста, не запуская loop). Проще: пишите коллбэки того же типа, что и тул.
...
- **Ошибка валидации Pydantic** при `many=True` — `@pin` **кидает исключение, не маскируя его**. LangGraph получает error от тула, агент сам решает что делать. Граф не меняется, `response_transform` не вызывается, `IngestResult` не создаётся. Принцип: fail loud, не тихое деградирование.
...
- **Exception в нормализаторе** — если `Entity.normalizer` кинул исключение (например, `canonical_ip("not-an-ip")` → `ValueError`), `@pin` пропускает исключение наверх, ingestion проваливается, граф не меняется. Нормализатор — пользовательский код, если он падает на данных, это сигнал, что либо нормализатор некорректный, либо данные кривые; fail-loud даёт явный диагноз вместо молчаливого warning.
- **`runtime.store is None`** — если граф собран без `compile(store=...)`, `@pin` кидает `AgentPinBoardConfigError("graph must be compiled with .compile(store=...) to use @pin-decorated tools")` на первом же вызове. Защита от типичного забытого аргумента в пользовательской сборке агента.
...
Все fail-loud сценарии кидают подклассы `AgentPinBoardError` — пользовательский агент может ловить одной точкой. `AgentPinBoardConfigError` кидается при декорировании (ранний отказ), остальные — в runtime. Хуки ловят своё внутри (см. 11.1).

---

## 7. Нормализаторы значений

AgentPinBoard **не поставляет** готовых нормализаторов. `Entity.normalizer` — это `Callable[[Any], str]`, пользователь передаёт любую функцию. Ядро остаётся доменно-нейтральным.
...
```text
("agent_pinboard", thread_id, "nodes", node_id)              → FactNode | EventNode (JSON)
("agent_pinboard", thread_id, "edges", edge_id)              → FactEdge (JSON)
("agent_pinboard", thread_id, "entities")                    → session-registry (один blob, редко меняется)
...
def dump_session(store, thread_id: str) -> dict:
    return {
        "agent_pinboard_version": "0.1",    # версия библиотеки, не per-object schema
        "nodes": [i.value for i in store.search(("agent_pinboard", thread_id, "nodes"))],
        "edges": [i.value for i in store.search(("agent_pinboard", thread_id, "edges"))],
        "entities": store.get(("agent_pinboard", thread_id, "entities"), "registry"),
        "tool_calls": [i.value for i in store.search(("agent_pinboard", thread_id, "tool_calls"))],
    }
```
...
Namespace `("agent_pinboard", thread_id, "tool_calls")`. Каждый вызов `@pin`-декорированного тула добавляет запись:
...
**Рост лога.** Запись неограниченная — лог хранится целиком за сессию. Soft-limit задаётся глобальным конфигом AgentPinBoard:
...
| `get_evidence` | 3 | `(event_id)` | Полный raw return тула (требует `@pin(store_raw=True)`) |
...
from agent_pinboard import (
    ...
    OnDuplicate,              # StrEnum для @pin(on_duplicate=...): ALWAYS / SKIP / CACHE
...
- Конкретные Pydantic-модели источников (CloudTrail, Okta, GitHub Audit, VT, WHOIS, ...) — пишутся пользователем или поставляются как отдельные пакеты (`agent-pinboard-aws`, `agent-pinboard-enrichment-vt`).
...
- `@pin` overhead (ingestion-блок на событие с 5 фактами) — < 10 мс.
...
- `get_evidence(event_id)` + опция `@pin(store_raw=True)` для хранения полного return тула в отдельном namespace — для forensic-анализа.
...
- **Guardrails каскадного обогащения** (Exit-tool, hard max_turns) — REASEARCH §C. Это agent-level concern, не library: `max_iterations` — параметр LangGraph `create_agent`, Exit-tool — пользовательский тул в агенте. AgentPinBoard даёт repetition detection через `on_duplicate` и depth-limits в `explore`.
...
Уникальность ниши AgentPinBoard — комбинация трёх архитектурных решений, которой нет ни у одной из существующих библиотек (REASEARCH §E.1):
...
2. **Side-effect декоратор `@pin`** — прозрачное извлечение фактов при вызове тула без изменения return value. Во всех альтернативах memory API вызывается явно.
...
- **Graphiti** (Zep AI) — персистентный KG с LLM-extraction и bi-temporal моделью. Мощнее, но дороже на порядок (600K+ токенов на диалог) и требует LLM в runtime. AgentPinBoard делает другой tradeoff: детерминизм и дешевизна в обмен на гибкость.
...
- **AriGraph** (IJCAI 2025) — ближайший академический аналог: episodic + semantic подграфы, инкрементальное построение. AgentPinBoard заимствует терминологию и формулу ранжирования эпизодов; отличие — извлечение декларативное, не LLM-based.
- **LangGraph Store** — фундамент, на котором AgentPinBoard живёт. Store — плоский key-value, AgentPinBoard добавляет графовую семантику поверх.
