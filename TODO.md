...
1. **Spans оторваны от agent-trace.** `client.start_observation(...)`
   ...
   trace: "agent invoke"
     ...
     │   └─ pinboard.ingest    ← должно быть ЗДЕСЬ
     └─ LLM: next step
   ```

   Сейчас pinboard-spans рождаются рядом как disconnected traces —
   корреляция с LLM-reasoning'ом, породившим ingest, теряется.

2. **Два span'a на каждый @fact — шум.** `pinboard.ingest` +
   `pinboard.graph_snapshot` стреляют всегда парой (снимок дёргается
...
- **A. Один span `pinboard.ingest` на вызов**:
...
- [ ] **`@fact(store_raw=True)` + `get_evidence` — async-тестов нет.**
...
- [ ] **Typed `hooks` protocol.** `PinBoardHooks` сейчас — обычный
...
- [ ] **`configure(ingest_queue_maxsize=...)`** для `WebSocketHook`
  ...
  глобальным).
- [ ] **Примеры `pinboard-aws` / `pinboard-enrichment-vt` пакетов-плагинов** —
  демонстрация паттерна, как расширять PinBoard для конкретных
  источников.
